...
Define the `package level variable` and `func`
...
// define the package level variable can't use syntax sweet
var conferenceName string = "Go Conference"
const conferenceTickets int = 50
var remainingTickets uint = 50
var bookings []string
...
### Organize Code with Go Package

> organize code to same package

![](https://tva1.sinaimg.cn/large/e6c9d24egy1h5n8qemez7j21c00u0gqr.jpg)
...
> organize code to different package

![](https://tva1.sinaimg.cn/large/e6c9d24egy1h5n8wc3doqj21c00u079a.jpg)
...
![](https://tva1.sinaimg.cn/large/e6c9d24egy1h5n96vnoucj21c00u07a7.jpg)

### Maps

这段代码直接跑是无法运行的, 需要再导入一个`booking-app/helper`的包 (具体结构看上面一节笔记)
...
func printFirstNames() []string {
	firstNames := []string{}
	for _, booking := range bookings {
		firstNames = append(firstNames, booking["fristName"])
	}
	return firstNames
}
func getUserInput() (string, string, string, uint) {
...
func bookTicket(userTickets uint, firstName string, lastName string, email string)  {
	...
	// define the type is "map[string]string"
	// use the make func to build a empty map
...
这段代码直接跑是无法运行的, 需要再导入一个`booking-app/helper`的包 (具体结构看`上上`面一节笔记), 来补充`ValidateUserInput`方法
...
`Map`和`Struct`的区别在于, 我们定义的`Map`类型是这样的

```go
map[string][string]
```

他的值只能是`string`, 而`struct`类型则不同, 可以定义任意类型的值
